 ...
 *
 * <p>Visitorパターンは直和型に対するパターンマッチと実質的に等価です。
 * オリジナルのVisitorパターンでは直和型に対する単項演算のみが定義されていますが、
 * 本パッケージでは2項演算以上に抽象化されています。
 *
 ...
 *     <li>
 *         Mono, Di
 *         <p>applyメソッドの引数の個数はギリシャ数字で表されます。Dispatcherの直前に付きます。現在2変数までが定義されています。
 *     </li>
 *     <li>
 *         2, 3
 *         <p>扱う直和型に対する派生型の個数はアラビア数字で表されます。Dispatcherの直後に付きます。
 *     </li>
 *     <li>
 *         I, Abstract
 *         <p>インターフェース、又は骨格実装を表す型名の接頭辞です。無印の場合標準の不変な実装を表します。
 *     </li>
 * </ul>
 *
 ...
 * 例えば{@literal Option<Integer>}に対して値をインクリメントしたOptionを返すDispatcherは次のように利用できます。
 * <pre>{@code
