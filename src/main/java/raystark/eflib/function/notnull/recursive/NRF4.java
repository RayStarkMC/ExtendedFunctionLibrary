 ...
 * 再帰的に定義された四変数関数です。
 *
 * <p>このインターフェースは{@link NRF4#apply(T1, T2, T3, T4, NRF4)}を関数メソッドに持つ関数型インターフェースです。
 * <p>この関数はnullを扱いません。
 *
...
public interface NRF4<T1, T2, T3, T4, R> {
     ...
     *
     * <p>再帰的ラムダ式では{@link NTailCall#call}に渡すSupplierの中でselfを参照し、{@link NRF4#apply(T1, T2, T3, T4)}メソッドを呼び出してください。
     *
     * @param t1 第一引数
     * @param t2 第二引数
     * @param t3 第三引数
     ...
     *
     * <p>この実装では{@link NRF4#apply(T1, T2, T3, T4, NRF4)}メソッドに引数と関数自身を渡します。
     *
     * @param t1 第一引数
     * @param t2 第二引数
     * @param t3 第三引数
