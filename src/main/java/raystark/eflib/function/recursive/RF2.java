 ...
 * 再帰的に定義された二変数関数です。
 *
 * <p>このインターフェースは{@link RF2#apply(T1, T2, RF2)}を関数メソッドに持つ関数型インターフェースです。
 *
...
public interface RF2<T1, T2, R> {
     ...
     *
     * <p>再帰的ラムダ式では{@link TailCall#call}に渡すSupplierの中でselfを参照し、{@link RF2#apply(T1, T2)}メソッドを呼び出してください。
     *
     * @param t1 第一引数
     * @param t2 第二引数
     * @param self this参照
     ...
     *
     * <p>この実装では{@link RF2#apply(T1, T2, RF2)}メソッドに引数と関数自身を渡します。
     *
     * @param t1 第一引数
     * @param t2 第二引数
     * @return 再帰関数の末尾呼び出し
