 ...
 * 再帰的に定義された型T1の述語です。
 *
 * <p>このインターフェースは{@link RP1#test}を関数メソッドに持つ関数型インターフェースです。
 *
...
public interface RP1<T1> {
     ...
     *
     * <p>再帰的ラムダ式では{@link BooleanTailCall#call}に渡すSupplierの中でselfを参照し、{@link RP1#test}メソッドを呼び出してください。
     *
     * @param t1 第一引数
     * @param self this参照
     * @return 再帰関数の末尾呼び出し
     ...
     * 指定された引数でこの述語を評価します。
     *
     * <p>この実装では{@link RP1#test}メソッドに引数と関数自身を渡します。
     *
     * @param t1 第一引数
     * @return 再帰関数の末尾呼び出し
     */
     ...
     * {@link BooleanTailCall#evaluate}の実行は{@link P1#test}の実行まで遅延されます。
     *
