 ...
 *     <li>C: Consumer 型Tの引数を受け取り値を返さない手続きです。複数の引数を取る場合があり、関数名には引数の個数を表す数が続きます。</li>
 *     <li>S: Supplier 型Tのインスタンスを供給します。</li>
 *     <li>A: Action 引数を受け取らなず値を返さない手続きです。</li>
 * </ul>
 *
 * <p>2つ以上の引数を取る関数は部分適用を行うapplyメソッド、および引数の順番を交換するswapメソッドを持ちます。
 * applyメソッドでは引数の個数だけ部分適用を行い、残りの引数を待機する関数を返します。
 * swapメソッドは第一引数とその他の引数を好感します。swap2, swap3のようにメソッド名に交換する引数の序列を表す数字続きます。
 * 例えばswap2メソッドでは第一引数と第二引数を、swap3メソッドでは第一引数と第三引数を交換します。
 *
 * <p>Aを除くすべての関数は自身の型を他の型に変換するasメソッドを持ちます。メソッド名asには変換後の関数名が続きます。
 ...
 * これにより、合成関数を1行で書くことが出来るようになります。以下はF1の例です。
 *
 * <pre>{@code
 ...
 * また副作用を生む場合があります。しかし、明瞭なプログラムを書くためにC: ConsumerとA: Action, 及びS: Supplierを除く全ての関数は副作用を持たず、
